...
def _registered_entries():
    ...
    # A pass handed over by value is not in any registry, so the call sites
    # that name one still have to be read out of the source. The entry has to
    # carry the *defining* file: `_reaches_a_bag` walks functions in the
    # entry's file, and an entry keyed on the call site (server_args.py, where
    # `run_post_process_pass(self, fn)` is spelled) finds no function of that
    # name there and walks nothing -- which made this whole channel a no-op
