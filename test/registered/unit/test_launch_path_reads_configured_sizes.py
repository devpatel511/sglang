...
class TestLaunchPathsReadConfiguredSizes(CustomTestCase):
    def test_configured_sizes_hold_when_the_live_topology_disagrees(self):
        ...
        # No resolve_once() here: publishing projects the bags either way, and
        # `tp_size` is raw input, so the configured value is 2 whether or not
        # this record has been through the pipeline. That also keeps the test
        # meaningful at every commit in a series that moves *when* resolution
        # runs.
        ...
        # rather than listed here: hand-listing it is how the first draft of
        # this test named a getter that does not exist and "failed" on a
        # production path that was fine.
